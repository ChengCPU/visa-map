...
const Libya:React.FC = () => {
  ...
  const { libyaColor } = useContext(ColorContext)
  const { libyaDiff } = useContext(DiffContext)
...
<g className={'libya'} onMouseOver={() => setCountrySelect('libya')}>
  ...
  `}</style>
<defs>
  <pattern id="ly" x="0" y="0" width="1" height="1" viewBox="0 0 60 60">
    <image width="1500" height="1500" xlinkHref={gridCalculation(libyaColor)}/>
  </pattern>
</defs>
{(libyaDiff == 1) ?
