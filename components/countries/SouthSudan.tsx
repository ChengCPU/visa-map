...
const SouthSudan:React.FC = () => {
  ...
  const { southSudanColor } = useContext(ColorContext)
  const { southSudanDiff } = useContext(DiffContext)

return (
<g className={'southSudan'}>
  <style jsx>{`
  .southSudan {
  ...
  `}</style>
<defs>
  <pattern id="ss" x="0" y="0" width="1" height="1" viewBox="10 0 40 40">
    <image width="1500" height="1500" xlinkHref={gridCalculation(southSudanColor)}/>
  </pattern>
</defs>
{(southSudanDiff == 1) ?
