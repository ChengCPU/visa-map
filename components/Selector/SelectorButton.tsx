...
const SelectorButton:React.FC<Props> = ({ setOpenDrawer, select, setSelect, selectArray, num }) => {
  ...
  const passportTextCalculation = (n:number) => {
    return countriesWithPassports[passportsArray.indexOf(selectArray[n])]
  }
  
  return (
    <Button title={selectArray[num] == null ? null : passportTextCalculation(num).charAt(0).toUpperCase() + passportTextCalculation(num).slice(1)} onClick={() => {
      setOpenDrawer(true);
