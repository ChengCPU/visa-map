export default function visaFreeAreaSort(rankData:any, rankRef:any) {
  ...
  for(let i = 0; i < unsortedData.length; i++) {
    let current:string | number = unsortedData[i][8]
    let current2:(string | number)[] = unsortedData[i]
    let j:number

    for(j = i - 1; j >= 0 && unsortedData[j][8] > current; j--) {
      unsortedData[j + 1] = unsortedData[j]
      unsortedData[j + 1][8] = unsortedData[j][8]
    }

    unsortedData[j + 1] = current2;
    unsortedData[j + 1][8] = current;
  }

  const sortedData:(string | number)[][] = unsortedData.reverse()

  const visaOnArrivalEvisaCheck:Function = (x:number) => {
    if(sortedData[x]?.[2] < sortedData[x + 1]?.[2] && sortedData[x]?.[8] == sortedData[x + 1]?.[8]) {
  ...
  const freedomOfMovementCheck:Function = (x:number) => {
    if(sortedData[x]?.[7] < sortedData[x + 1]?.[7] && sortedData[x]?.[8] == sortedData[x + 1]?.[8]) {
