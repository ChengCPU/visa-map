...
function MyApp({ Component, pageProps }) {
  ...
  const visaPolicyData = useRef<any>()
  ...
  useEffect(() => {
    ...
    fetchSortData(visaPolicyData, setRankRender, 1, 'Descending')
  ...
  useEffect(() => {
    if(selectArray[select.selection] != null && selectArray[select.selection] != select.passport) { reset(setAssignedColors, priorityRef, tempPriorityRef, secondToggle, setSecondToggle, setDiff, setTempDiff); return }
    selectArrayCalculation(selectArray, setSelectArray, select);
    if(selectArray[select.selection] != null) { mainCalculation(selectArray[select.selection], assignedColors, setAssignedColors, select, priorityRef, selectArray, diff, setDiff, setPercentage) }
  }, [toggle])

  useEffect(() => {
    subCalculation(selectArray, assignedColors, setAssignedColors, priorityRef, diff, setDiff, setPercentage)
    selectArrayCalculation(selectArray, setSelectArray, select)
  }, [secondToggle])
